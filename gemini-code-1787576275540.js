 ...
 *  (using Google's Gemini API & Real Razorpay Payments)
 * ============================================================================
 *
...
// ----------------------------------------------------------------------------
// PDF THEME — Elegant Gold theme (cream/parchment + gold + charcoal)
// ----------------------------------------------------------------------------
...
// ----------------------------------------------------------------------------
// FRONTEND — Tailwind CSS + Razorpay Checkout Integration
// ----------------------------------------------------------------------------
    ...
    <div class="bg-wineLight/40 border border-hairline rounded-2xl p-8 shadow-2xl backdrop-blur-sm">
      <form id="resumeForm" class="space-y-6">
        ...
        <div class="mt-8 rounded-2xl border border-hairline bg-wine/60 p-6">
          ...
          <p class="text-center text-xs text-parchment/50 mt-3">256-bit Encrypted Checkout via Razorpay</p>
        </div>
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      const options = {
        ...
        description: 'Executive ATS Résumé Generation',
        order_id: order.id,
        prefill: {
          name: data.fullName,
          email: data.email,
        ...
        handler: async function (response) {
          ...
          try {
            // Step 3: Verify signature and generate resume PDF
            ...
            if (!pdfRes.ok) {
              ...
              throw new Error(err.error || 'Failed to build résumé.');
            }
            ...
            a.href = url;
            a.download = (data.fullName || 'resume').replace(/\\s+/g, '_') + '_Resume.pdf';
...
// ----------------------------------------------------------------------------
// ROUTE: /api/create-order (Creates Razorpay Order Server-Side)
// ----------------------------------------------------------------------------
app.post('/api/create-order', async (req, res) => {
  try {
    if (!process.env.RAZORPAY_KEY_ID || !process.env.RAZORPAY_KEY_SECRET) {
      return res.status(500).json({ error: 'Razorpay API keys missing in environment variables.' });
    }
...
// ----------------------------------------------------------------------------
// REAL HMAC-SHA256 RAZORPAY PAYMENT VERIFICATION
// ----------------------------------------------------------------------------
...
// ----------------------------------------------------------------------------
// PDF RENDERING — Harvard Layout with Elegant Gold Accent
// ----------------------------------------------------------------------------
const MARGIN = 36;
...
app.post('/api/generate-resume', async (req, res) => {
  try {
    ...
    const paymentOk = verifyRazorpayPayment(body);
    if (!paymentOk) {
