...
// ----------------------------------------------------------------------------
// PDF THEME — single design now (cream/parchment + gold + charcoal).
// The three old options (Classic Executive / Modern Minimal / Tech Minimalist)
// have been removed and replaced with this one "Elegant Gold" theme.
// NOTE: wine (#4A1620) is the WEBSITE's color only — it is intentionally NOT
// used anywhere in the PDF itself.
// ----------------------------------------------------------------------------
const ELEGANT_GOLD_THEME = {
  pageBg: '#F7F1E6',      // cream / parchment
  accent: '#A5824F',      // gold (slightly deepened for on-paper legibility)
  divider: '#C9A876',     // lighter gold, for rule lines
  heading: '#2A2018',     // dark charcoal (name, job titles)
  subtext: '#6B5F4F',     // muted brown-grey (dates, org lines)
  body: '#3A3226',        // body text, warm dark tone (not pure black)
  headerFont: 'Times-Bold',
  bodyFont: 'Times-Roman',
};

function getTheme() {
  // Kept as a function (rather than a flat constant) so the rest of the PDF
  // code below doesn't need to change shape — it just always resolves to
  // the one Elegant Gold theme now.
...
<link href="https://fonts.googleapis.com/css2?family=Cormorant:ital,wght@0,500;0,600;1,500&family=Libre+Franklin:wght@400;500&display=swap" rel="stylesheet">
<style>
  body {
...
<body class="min-h-screen">
  ...
  <div class="max-w-3xl mx-auto px-6 py-14">
    ...
    <div class="mb-14 text-center">
      ...
      <h1 class="hero-heading headline-cream text-3xl md:text-5xl font-semibold tracking-tight leading-tight max-w-2xl mx-auto">
        You're not building a document. You're building the first five seconds of someone's impression of you — make those five seconds count.
      </h1>
      <div class="hero-rule"></div>
      <p class="hero-subheading font-display italic text-gold text-base md:text-lg">
...
4. QUANTIFIABLE METRICS: Wherever the raw input implies scale, impact, or improvement but supplies no figure, insert a bracketed placeholder such as "[X]%", "[X]+ team members", "[X]K users", or "$[X]K" so the candidate may supply the true figure later. Never invent a number as though it were factual.
...
7. Only use information present in, or reasonably inferable from, the candidate's raw input. Do not fabricate job titles, companies, or dates that were not provided — if a field such as company name or dates is missing, omit it or use a neutral placeholder like "[Company Name]" / "[Dates]".
...
function generatePdfBuffer(resume) {
  return new Promise((resolve, reject) => {
    ...
    doc.moveDown(0.5);
    drawDivider(doc, theme, false); // short centered gold rule, Harvard-style
